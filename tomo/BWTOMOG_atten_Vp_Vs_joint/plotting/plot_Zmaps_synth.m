...
try
    [topoX,topoY,topoZ] = grdread2('/Users/zeilon/Work/EastAfrica/EARdb/PLOTTING/EAR_etopo1_bedrock.grd'); % load topo grid
    [topoX,topoY] = meshgrid(double(topoX),double(topoY)); topoZ = double(topoZ);
catch
    warning('no etopo data');
end
...
% Stations
hs = plot(data.stn.lon,data.stn.lat,'v');
...
for ixy = 1:size(par.zsectxy,3)
    ...
    set(gcf, 'Color', [1 1 1])
    if ifsave
        save2pdf(15,strcat(typestr{par.t_ts},'_Zmap_syn',iostr{io},'_',num2str(ixy)),par.figdir);
    end
...
%% Save hmap
if ifsave
    save2pdf(14,strcat(typestr{par.t_ts},'_Zmap_syn',iostr{io},'_hplot'),par.figdir);
end
