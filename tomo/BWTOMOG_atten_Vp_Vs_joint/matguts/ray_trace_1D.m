function [rx,ry,rz,Dr] = ray_trace_1D(par, p, baz, slat, slon, selev, ssed, p_or_s)
% [rx,ry,rz,Dr] = ray_trace_1D(par, p, baz, slat, slon, selev, ssed)
% 
% 1D ray tracing function

if nargin < 8 || isempty(p_or_s)
    if par.PS <3 
        p_or_s = par.PS;
    else
        error('Need to specify if P or S wave');
    end
end
...
v1  = vel_profile(p_or_s,zz1,slat,slon,selev,false,ssed); % to make contingent on path
v2  = vel_profile(p_or_s,zz2,slat,slon,selev,false,ssed);
r1 = 6371-zz1;
r2 = 6371-zz2;
